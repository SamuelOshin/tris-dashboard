...
def test_full_tris_e2e_browser_flow():
    ...
    with sync_playwright() as p:
        ...
        # Wait for SPA redirect to Dashboard
        ...
        print(
            "[PASS] Supplier Baseline Drawer verified ($30,471.43 mean strictly excluding TX-1999)"
        )

        # Close drawer
        page.locator('button:has-text("Close Baseline Inspector")').click()
        ...
        print("\n--- 5. Testing Governed State Transitions, Reopen Flow, & 8-Field Closure ---")
        ...
        for step in range(8):
            ...
            print(f"Step {step}: Visible buttons = {buttons}")
            if page.locator('button:has-text("Verified Closure (8 Fields)")').is_visible():
                break
            if page.locator('button:has-text("Reopen Case")').is_visible():
                print("Case currently Closed — testing Reopen Case modal")
                page.locator('button:has-text("Reopen Case")').click()
                page.wait_for_selector("text=Reopen Case for Inquiry", timeout=5000)
                page.locator("#reopen_reason").fill(
                    "Inconsistent callback evidence received; reopening for active inquiry"
                )
                page.locator('button:has-text("Confirm Reopen")').click()
                page.wait_for_selector("text=Case Reopened for Active Inquiry", timeout=8000)
                print("[PASS] Reopen Case modal and Reopened banner verified")
            elif page.locator('button:has-text("Resume Investigation")').is_visible():
                page.locator('button:has-text("Resume Investigation")').click()
            elif page.locator('button:has-text("Assign Case")').is_visible():
                page.locator('button:has-text("Assign Case")').click()
            elif page.locator('button:has-text("Begin Investigation")').is_visible():
                page.locator('button:has-text("Begin Investigation")').click()
            elif page.locator('button:has-text("Initiate Corrective Action")').is_visible():
                page.locator('button:has-text("Initiate Corrective Action")').click()
            elif page.locator('button:has-text("Submit for Verification")').is_visible():
                page.locator('button:has-text("Submit for Verification")').click()
            elif page.locator('button:has-text("Submit for Re-Verification")').is_visible():
                page.locator('button:has-text("Submit for Re-Verification")').click()
        ...
        page.locator('button:has-text("Attest & Seal Case")').click()
        expect(page.locator("text=Incomplete Attestation")).to_be_visible()
        ...
        page.locator("#closure_corrective_action").fill(
            "Bank details reverted and payment hold placed on invoice NC-260828"
        )
        page.locator("#closure_evidence").fill(
            "Audit ticket SEC-2026-881; direct callback confirmation with supplier CFO"
        )
        ...
        page.locator("#closure_follow_up").fill(
            "Mandatory retest of supplier bank details in 14 days"
        )
        page.locator("#closure_recurrence_monitoring").fill(
            "Active 90-day surveillance on supplier SUP-001"
        )
        ...
        # Verify Closed Seal
        expect(page.locator("text=Verified Closure Compliance Certificate")).to_be_visible()
        ...
        page.screenshot(
            path=str(SCREENSHOTS_DIR / "11_acceptance_matrix_dashboard.png"), full_page=True
        )
